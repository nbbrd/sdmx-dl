...
public class ConnectionAssert {
    ...
    public void assertCompliance(SoftAssertions s, ConnectionSupplier supplier, Sample sample) {
        ...
        assertState(s, supplier, o -> o.getData(NO_CATALOG, sample.validFlow, Query.ALL), "getData(DataflowRef, DataQuery)");
        assertState(s, supplier, o -> o.getDataStream(NO_CATALOG, sample.validFlow, Query.ALL), "getDataStream(DataflowRef, DataQuery)");
        assertState(s, supplier, o -> o.getStructure(NO_CATALOG, sample.validFlow), "getStructure(DataflowRef)");
        assertState(s, supplier, o -> o.getFlow(NO_CATALOG, sample.validFlow), "getFlow(DataflowRef)");
        assertState(s, supplier, o -> o.getFlows(NO_CATALOG), "getFlows()");
    }
    ...
    private void checkValidFlow(SoftAssertions s, Sample sample, Connection conn) throws IOException {
        ...
        s.assertThat(conn.getFlows(NO_CATALOG))
                .are(validDataflow())
                .anyMatch(sample.validFlow::containsRef);

        s.assertThat(conn.getFlow(NO_CATALOG, sample.validFlow))
                .is(validDataflow());

        Structure dsd = conn.getStructure(NO_CATALOG, sample.validFlow);
    ...
    private void checkInvalidKey(SoftAssertions s, Sample sample, Connection conn, Detail filter) {
        ...
        s.assertThatThrownBy(() -> conn.getData(NO_CATALOG, sample.validFlow, invalidQuery))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContainingAll("Expecting key", sample.invalidKey.toString());

        s.assertThatThrownBy(() -> conn.getDataStream(NO_CATALOG, sample.validFlow, invalidQuery))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContainingAll("Expecting key", sample.invalidKey.toString());
    }
    ...
    private void checkValidKey(SoftAssertions s, Sample sample, Connection conn, Detail filter) throws IOException {
        ...
        s.assertThat(conn.getDataStream(NO_CATALOG, sample.validFlow, validQuery))
                .containsExactlyElementsOf(conn.getData(NO_CATALOG, sample.validFlow, validQuery).getData());
    }
    ...
    private void checkInvalidFlow(SoftAssertions s, Sample sample, Connection conn) {
        for (Detail filter : Detail.values()) {
            ...
            s.assertThatThrownBy(() -> conn.getData(NO_CATALOG, sample.invalidFlow, validQuery))
                    .isInstanceOf(IOException.class);

            s.assertThatThrownBy(() -> conn.getDataStream(NO_CATALOG, sample.invalidFlow, validQuery))
                    .isInstanceOf(IOException.class);

            s.assertThatThrownBy(() -> conn.getFlow(NO_CATALOG, sample.invalidFlow))
                    .isInstanceOf(IOException.class);

            s.assertThatThrownBy(() -> conn.getStructure(NO_CATALOG, sample.invalidFlow))
                    .isInstanceOf(IOException.class);
        }
    }
    ...
    private void assertNonnull(SoftAssertions s, Connection conn, FlowRef ref) {
        s.assertThatThrownBy(() -> conn.getData(NO_CATALOG, null, Query.ALL))
                .as(nullDescriptionOf("getData(DataflowRef, DataQuery)", "flowRef"))
                .isInstanceOf(NullPointerException.class);

        s.assertThatThrownBy(() -> conn.getData(NO_CATALOG, ref, null))
                .as(nullDescriptionOf("getData(DataflowRef, DataQuery)", "query"))
                .isInstanceOf(NullPointerException.class);

        s.assertThatThrownBy(() -> conn.getDataStream(NO_CATALOG, null, Query.ALL))
                .as(nullDescriptionOf("getDataStream(DataflowRef, DataQuery)", "flowRef"))
                .isInstanceOf(NullPointerException.class);

        s.assertThatThrownBy(() -> conn.getDataStream(NO_CATALOG, ref, null))
                .as(nullDescriptionOf("getDataStream(DataflowRef, DataQuery)", "query"))
                .isInstanceOf(NullPointerException.class);

        s.assertThatThrownBy(() -> conn.getStructure(NO_CATALOG, null))
                .as(nullDescriptionOf("getStructure(DataflowRef)", "flowRef"))
                .isInstanceOf(NullPointerException.class);

        s.assertThatThrownBy(() -> conn.getFlow(NO_CATALOG, null))
