 ...
 * </p>
 */
public final class DumpingHttpClientDecorator implements HttpClientDecorator {
    ...
    private final HttpClientDecorator support = HttpClientDecoratorSupport.builder()
            ...
            .superFactory(DumpingHttpClientDecorator::decorate)
            .build();
     ...
     * a {@link DumpingHttpClient} that copies downloaded content to disk. Events are reported
    ...
    private static HttpClient decorate(HttpClientFactory d, WebSource s, WebContext c) {
        ...
        if (dumpFolder != null) {
            ...
            return new DumpingHttpClient(dumpFolder.toPath(), original, onEvent != null ? file -> onEvent.accept("RI_HTTP", "Dumping " + file.toUri()) : ignore -> {
            });
        }
        return original;
    }
