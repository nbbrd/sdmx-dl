...
public final class BasicAuthenticator implements Authenticator {
    ...
    // NB: static so that all the instances of this authenticator are mutually excluded on the same resource
    private static final KeyedLock LOCKS = KeyedLock.newInstance();

    private final Duration ttl = Duration.ofMinutes(5);
    private final Duration lockTimeout = Duration.ofMinutes(2);
    ...
    @Override
    public PasswordAuthentication getPasswordAuthenticationOrNull(
        ...
        if (isBasicAuthScheme(source)) {
            ...
            try (KeyedLock.Lease lease = LOCKS.tryAcquire(resource, lockTimeout)) {
                if (lease == null) {
                    if (onError != null)
                        onError.accept(
                                getAuthenticatorId(),
                                "Failed to acquire username/password for resource '" + resource + "'",
                                new IOException("Timeout while waiting for resource '" + resource + "'"));
                    return null;
                }
                ...
                if (onEvent != null)
                    onEvent.accept(
                            getAuthenticatorId(),
                            "Acquiring username/password silently for resource '" + resource + "'");
                ...
                if (onEvent != null)
                    onEvent.accept(
                            getAuthenticatorId(),
                            "Acquiring username/password interactively from '" + passwordPrompt.getPromptId()
                                    + "' for resource '" + resource + "'");
                PasswordAuthentication passwordAuthentication =
                        passwordPrompt.promptCredentials(resource, "Enter your credentials for " + source.getId());
                if (passwordAuthentication != null) {
                    cache.put(
                            resource,
                            fromPasswordAuthentication(resource, passwordAuthentication, cache.getClock(), ttl));
                ...
                if (onError != null)
                    onError.accept(
                            getAuthenticatorId(),
                            "Failed to acquire username/password for resource '" + resource + "'",
                            new IOException("Interrupted while waiting for resource '" + resource + "'", ex));
            }
        }
        return null;
    }
    ...
    private static Credentials fromPasswordAuthentication(
            String resource, PasswordAuthentication passwordAuthentication, Clock clock, Duration ttl) {
        String secret = passwordAuthentication.getUserName() + ":" + new String(passwordAuthentication.getPassword());
        return Credentials.of(
                new PasswordAuthentication(resource, secret.toCharArray()),
