...
public final class MsalAuthenticator implements Authenticator {
    ...
    @PropertyDefinition
    public static final Property<URI> REDIRECT_URI_PROPERTY = Property.of(
            AUTHENTICATOR_PROPERTY_PREFIX + ".redirectUri",
    ...
    // NB: static so that all the instances of this authenticator are mutually excluded on the same UID
    private static final KeyedLock LOCKS = KeyedLock.newInstance();
    ...
    @Override
    public @Nullable PasswordAuthentication getPasswordAuthenticationOrNull(
        ...
        if (config != null) {
            IPublicClientApplication app = PublicClientApplication.builder(config.getClientId())
                    ...
                    .build();
            return newToken(acquireToken(app, config.getScopes(), config.getRedirectUri(), onEvent, config.getUid())
                    .accessToken());
        }
        return null;
    }
    ...
    public @NonNull Collection<String> getAuthenticatorPropertyNames() {
        return keysOf(UID_PROPERTY, CLIENT_ID_PROPERTY, AUTHORITY_PROPERTY, SCOPES_PROPERTY, REDIRECT_URI_PROPERTY);
    }
    ...
    static class MsalConfig {
        ...
        public static @Nullable MsalConfig parse(@NonNull WebSource source) throws IOException {
            if (AuthSchemes.MSAL_AUTH_SCHEME.equals(
                    DriverProperties.AUTH_SCHEME_PROPERTY.get(source.getProperties()))) {
        ...
        private static <T> @NonNull T getNotNull(@NonNull Property<T> property, @NonNull WebSource source)
                throws IOException {
    ...
    private static IAuthenticationResult acquireToken(
            IPublicClientApplication app, Set<String> scopes, URI redirectUri, EventListener onEvent, String uid)
            throws IOException {
        try (KeyedLock.Lease lease = LOCKS.tryAcquire(uid, LOCK_TIMEOUT)) {
            if (lease == null) {
                throw new IOException("Timeout while waiting for UID '" + uid + "'");
            }
            // NB: the interactive fallback must run while holding the lock
            try {
                if (onEvent != null) onEvent.accept(ID, "Acquiring token silently for UID '" + uid + "'");
                return app.acquireTokenSilently(SilentParameters.builder(scopes)
                                .account(app.getAccounts().join().stream()
                                        .findFirst()
                                        .orElse(null))
                                .build())
                        .join();
                ...
                if (ex.getCause() instanceof MsalException) {
                    ...
                    return app.acquireToken(InteractiveRequestParameters.builder(redirectUri)
                                    .scopes(scopes)
            ...
            throw new IOException("Interrupted while waiting for UID '" + uid + "'", ex);
        }
    }
    ...
    private static final class CachedTokenCacheAccessAspect implements ITokenCacheAccessAspect {
        ...
        public void beforeCacheAccess(ITokenCacheAccessContext context) {
            ...
            if (token != null && !token.isEmpty())
                context.tokenCache()
                        .deserialize(String.valueOf(token.getCredentials().getPassword()));
        }
                ...
                cache.put(
                        ...
                        Credentials.of(
                                new PasswordAuthentication(
                                        uid, context.tokenCache().serialize().toCharArray()),
                                cache.getClock().instant().plus(ttl)));
            }
        }
    }
}
